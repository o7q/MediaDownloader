...
export function generateIPCConfig(): IPCConfig {
    ...
    const settingsFormatDropdown = document.getElementById("settings-format-dropdown") as HTMLSelectElement;
    let formatType = "null";
    if (settingsFormatDropdown) {
        const selectedOption = settingsFormatDropdown.options[settingsFormatDropdown.selectedIndex];
    ...
    const config: IPCConfig = {
        ...
        settings: {
            format:                         ($("settings-format-dropdown")             as HTMLSelectElement)  .value,
